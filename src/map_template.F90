module map_VAL_NAME_m
	...
	type map_entry_VAL_NAME_t
		character(len=:), allocatable :: key
		VAL_OUT_ :: val
	end type map_entry_VAL_NAME_t

	!********
	type map_VAL_NAME_t
		type(map_entry_VAL_NAME_t), allocatable :: entries(:)
		integer(kind=8) :: len_ = 0
		contains
			procedure :: set => set_map_VAL_NAME
			procedure :: get => get_map_VAL_NAME
			procedure, private :: set_core => set_map_VAL_NAME_core
	end type map_VAL_NAME_t
...
contains
	...
	function new_map_VAL_NAME(cap) result(map)
		! Create a new empty map
		type(map_VAL_NAME_t) :: map
	...
	end function new_map_VAL_NAME

	recursive subroutine set_map_VAL_NAME(this, key, val)
		! Set a key-value pair in the map
		class(map_VAL_NAME_t), intent(inout) :: this
		character(len=*), intent(in) :: key
		VAL_IN__, intent(in) :: val
		!********
		integer(kind=8) :: i, n, n0
		type(map_entry_VAL_NAME_t), allocatable :: old_entries(:)
	...
	end subroutine set_map_VAL_NAME

	subroutine set_map_VAL_NAME_core(this, key, val)
	...
	end subroutine set_map_VAL_NAME_core

	function get_map_VAL_NAME(this, key, found) result(val)
		! Get a value by key from the map
		class(map_VAL_NAME_t), intent(in) :: this
		character(len=*), intent(in) :: key
		...
		logical :: found_
		found_ = .false.
		val = VAL_DFLT  ! Default value if not found
		hash = djb2_hash(key)
	...
	end function get_map_VAL_NAME

end module map_VAL_NAME_m
